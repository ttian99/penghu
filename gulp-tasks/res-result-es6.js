...
function getResObj() {
  ...
  _.keys(cfg.resGroup).forEach(gName => {
    ...
    const {ani} = cfg.resGroup[gName];
    if (ani) ani.forEach((aniInfo) => aniArr.push(aniInfo.name));
  });
  ...
  // 序列帧动画
  res.ani = {};
  aniArr.forEach((name) => res.ani[name] = name);
...
async function createResJs(isMini) {
  const rst = {
  ...
  const filename = 'lib/loader/resource.' + (isMini ? 'mini' : 'normal') + '.js';
  await fs.writeFile(filename, cpl);
}

export default async function resResult(src, dest, needHash = true, isMini = false) {
